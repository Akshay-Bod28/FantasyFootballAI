...
data = pd.read_csv("app/data/qb_stats.csv") 
...
y = data["top_10_next_year"]   #Column of if they were top 10 the next season or not
...
X_train, Y_train = train[feature_cols], train["top_10_next_year"]  #Train model on the seasons before 2024 with the stat groups I picked to see if they were top 10 the next season or not
X_test, Y_test = test[feature_cols], test["top_10_next_year"]      #Test to see if the model can predict 2024 season
...
joblib.dump(model, "app/saved_models/qb_model.pkl")